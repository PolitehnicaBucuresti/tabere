...
export function AdminApplicationsGrouped({ rows, onRemove }: Props) {
  const rankById = useMemo(() => buildRegistrationRankByApplicationId(rows), [rows]);
          ...
          <section key={`${group.ageCategory}-${group.series}`} className="adminCard adminGroupSection">
            <h2 className="adminGroupHeading">
              ...
              {group.applications.length > 0 ? (
                <span className="adminGroupSubcounts">
                  {" "}
                  ({confirmed} înscriși, {waitlist} listă așteptare)
                </span>
              ) : null}
            </h2>
                ...
                <table className="adminTable">
                  ...
                  <tbody>
                    {group.applications.map((r, i) => {
                      const status = resolveRegistrationStatus(r, rankById);
                      const isWaitlist = status === REGISTRATION_STATUS_WAITLIST;
                      return (
                        <tr key={r.id}>
                          ...
                          <td>
                            <span
                              className={`adminStatusBadge ${isWaitlist ? "adminStatusBadgeWaitlist" : "adminStatusBadgeConfirmed"}`}
                            >
                              {status}
                            </span>
                          </td>
